...
gscv_lasso = GridSearchCV(estimator=Lasso(alpha=0.0005),  param_grid=param_grid_lasso, cv=5)
gscv_lasso.fit(train_x, train_y)
gscv_lasso.grid_scores_, gscv_lasso.best_params_, gscv_lasso.best_score_
preds_lasso = np.expm1(gscv_lasso.predict(train_x))

# {'max_depth': 2, 'min_child_weight': 1},0.9170547364376114)
param_grid_xgb = {
 'max_depth': [2],
 'min_child_weight': [1]
}
...
preds = 0.7 * preds_lasso + 0.3 * preds_xgb
# plt.scatter(preds_lasso, preds_krr)
plt.figure()
plt.scatter(np.expm1(train_y), preds)
print(np.sqrt(metrics.mean_squared_error(np.expm1(train_y), preds_xgb)))
