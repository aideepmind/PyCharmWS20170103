...
# # 合并 pd.merge
# all_data = pd.merge(all_data, ngbr_details, how='right', on='Neighborhood', sort=False, right_index=True)
# # 按Neighborhood分组
...
# all_data = pd.merge(all_data, temp, how='right', on='Neighborhood', sort=False, right_index=True)
...
# 再使用rf训练
#  {'max_depth': 4, 'min_samples_split': 10}, 0.9634236660795701)
param_grid_rf = {
    'min_samples_leaf': [5]
}
rf = GridSearchCV(
...
# plt.scatter(preds_lasso, preds_gbm)
# plt.scatter(rf.predict(train_x), train_y)
...
result.to_csv("kaggle/housePrices/temp/lasso_gmb_rf_newdata_test_result.csv", index=False)
