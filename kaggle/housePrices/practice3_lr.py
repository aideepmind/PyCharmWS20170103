...
param_grid = {
    "fit_intercept": [True]}
lr = GridSearchCV(estimator=LinearRegression(), cv=5, param_grid=param_grid)
lr.fit(train_x, train_y)
lr.grid_scores_, lr.best_params_, lr.best_score_
preds_lr = np.expm1(lr.predict(test_x))
result = pd.DataFrame({"Id": test_id, "SalePrice": preds_lr})
sns.distplot(result['SalePrice'], fit=norm)
result.to_csv("kaggle/housePrices/temp/lr_test_result.csv", index=False)


preds_lr = np.expm1(lr.predict(train_x))
result = pd.DataFrame({"Id": train_data['Id'], "SalePrice": preds_lr})
result.to_csv("kaggle/housePrices/temp/lr_train_result.csv", index=False)