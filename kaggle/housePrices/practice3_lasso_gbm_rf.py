...
print('preds_lasso:', np.sqrt(metrics.mean_squared_error(train_y, preds_lasso)))
# result = pd.DataFrame({"Id": test_id, "SalePrice": np.expm1(lasso.predict(test_x))})
# sns.distplot(result['SalePrice'], fit=norm)
# result['SalePrice'][result['Id'] == 2550] = 235594.51145508
# result.to_csv("kaggle/housePrices/temp/lasso_test_result3.csv", index=False)
...
# coef = pd.Series(krr.best_estimator_.coef_, index = train_x.columns)
# coef.sort_values(ascending=False).head(20)
preds_krr = krr.predict(train_x)
print('preds_krr:', np.sqrt(metrics.mean_squared_error(train_y, preds_krr)))
...
# preds = 0.7 * preds_lasso + 0.3 * preds_gbm
# plt.scatter(preds_lasso, preds_gbm)
...
preds_gbm_df = pd.DataFrame(preds_gbm, columns=['preds_gbm_df'], index=train_x.index)
train_x_concat = pd.concat((train_x, preds_lasso_df), axis=1)
train_x_concat = pd.concat((train_x_concat, preds_krr_df), axis=1)
train_x_concat = pd.concat((train_x_concat, preds_gbm_df), axis=1)

# 再使用rf训练
#  {'max_depth': 4, 'min_samples_split': 10}, 0.9690500913050346
param_grid_rf = {
    'max_depth': [5]
}
rf = GridSearchCV(
    ...
    iid=False,
    cv=5)
rf.fit(train_x_concat, train_y)
rf.grid_scores_, rf.best_params_, rf.best_score_
preds_rf = np.expm1(rf.predict(train_x_concat))
# plt.scatter(preds_lasso, preds_gbm)
plt.figure()
plt.scatter(rf.predict(train_x_concat), preds_krr_df)
plt.xlim([10, 14])
plt.ylim([10, 14])
print('rf.best_score_:', rf.best_score_)
print('preds_rf:', np.sqrt(metrics.mean_squared_error(train_y, rf.predict(train_x_concat))))

# 预测
preds_lasso_test = lasso.predict(test_x)
preds_krr_test = krr.predict(test_x)
preds_gbm_test = gbm.predict(test_x)
preds_lasso_df_test = pd.DataFrame(preds_lasso_test, columns=['preds_lasso_df'], index=test_x.index)
preds_krr_df_test = pd.DataFrame(preds_krr_test, columns=['preds_krr_df'], index=test_x.index)
preds_gbm_df_test = pd.DataFrame(preds_gbm_test, columns=['preds_gbm_df'], index=test_x.index)
test_x_concat = pd.concat((test_x, preds_lasso_df_test), axis=1)
test_x_concat = pd.concat((test_x_concat, preds_krr_df_test), axis=1)
test_x_concat = pd.concat((test_x_concat, preds_gbm_df_test), axis=1)
preds_test = np.expm1(rf.predict(test_x_concat))
result = pd.DataFrame({"Id": test_id, "SalePrice": preds_test})
plt.figure()
plt.scatter(rf.predict(test_x_concat), preds_lasso_test)
plt.xlim([10, 14])
plt.ylim([10, 14])
# result['SalePrice'][result['Id'] == 2550] = 235594.51145508
result.to_csv("kaggle/housePrices/temp/lasso_krr_rf_test_result.csv", index=False)


# 分类fit
n_clusters = 3
SalePrice = pd.DataFrame(train_y, columns=['SalePrice'], index=train_y.index)
km = KMeans(n_clusters=n_clusters).fit_predict(SalePrice)
SalePrice_Cls = pd.DataFrame({'Id': train_data['Id'], 'SalePrice': train_y, 'Cls': km}, index=train_data.index)
# sns.boxplot(x='Cls', y="SalePrice", data=SalePrice_Cls)
# 分类训练
param_grid_rf = {
    'max_depth': [100]
}
rf = GridSearchCV(
    ...
    iid=False,
    cv=5)
rf.fit(train_x, SalePrice_Cls['Cls'])
rf.grid_scores_, rf.best_params_, rf.best_score_
preds_rf = rf.predict_proba(train_x)
...
# imp.to_csv("kaggle/housePrices/temp/feature_importances3.csv", index=False)
# preds_rf = rf.predict(train_x)

# 使用Lasso训练分类数据
cls_ids = SalePrice_Cls[SalePrice_Cls['Cls'] == 2].index
np.corrcoef(train_y.loc[cls_ids], train_x.loc[cls_ids]['OverallQual'])
#
# {'alpha': 0.0007}, 0.7917808417693989)
param_grid_lasso = {
    'alpha': [0.0005, 0.0006, 0.0007, 0.0008, 0.0009]
}
lasso = GridSearchCV(estimator=Lasso(alpha=0.0005),  param_grid=param_grid_lasso, cv=5)
lasso.fit(train_x.loc[cls_ids], train_y.loc[cls_ids])
lasso.grid_scores_, lasso.best_params_, lasso.best_score_
preds_lasso = lasso.predict(train_x.loc[cls_ids])
print('preds_lasso:', np.sqrt(metrics.mean_squared_error(train_y.loc[cls_ids], preds_lasso)))