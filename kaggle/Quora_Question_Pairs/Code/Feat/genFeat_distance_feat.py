...
def JaccardCoef(A, B):
    A, B = set(A), set(B)
    intersect = len(A.intersection(B))  # intersection: 交集
    union = len(A.union(B)) # union: 并集
        ...
        for j in range(config.n_classes):
            ...
            if indices_dict.has_key(key):
                ...
                if len(distance_tmp) != 0:
                    ...
                    quantiles = distance_tmp.quantile(quantiles_range)  # quantile：根据正态分布找出给出range的点，range一般取值0最小数，0.5中位数，1最大数
                    feat = np.hstack((feat, quantiles)) # 垂直合并（叠加），与vstack相反，还有stack
                    stats_feat[i,j*stats_feat_num:(j+1)*stats_feat_num] = feat
    return stats_feat

# 参考文档，但不知道statistic的根据是什么
# 这段代码应该有个问题，感觉Train Set应该跟Valid Set合并计算
# 况且，这里没有涉及到Test set，不知Test Set是怎么处理的
        ...
        for name in ["title", "description"]:
            for gram in ["unigram", "bigram", "trigram"]:
                ...
                with open("%s/train.%s_%s_%s_stats_feat_by_relevance.feat.pkl" % (path, name, gram, dist), "wb") as f:
                    dill.dump(dist_stats_feat_by_relevance_train, f, -1)
                with open("%s/train.%s_%s_%s_stats_feat_by_query_relevance.feat.pkl" % (path, name, gram, dist), "wb") as f:
                    dill.dump(dist_stats_feat_by_query_relevance_train, f, -1)
                ## test
                ...
                with open("%s/%s.%s_%s_%s_stats_feat_by_relevance.feat.pkl" % (path, mode, name, gram, dist), "wb") as f:
                    dill.dump(dist_stats_feat_by_relevance_test, f, -1)
                with open("%s/%s.%s_%s_%s_stats_feat_by_query_relevance.feat.pkl" % (path, mode, name, gram, dist), "wb") as f:
                    dill.dump(dist_stats_feat_by_query_relevance_test, f, -1)
...
if __name__ == "__main__":
    ...
    ## stats to extract
    quantiles_range = np.arange(0, 1.5, 0.5)    # array([ 0. ,  0.5,  1. ])
