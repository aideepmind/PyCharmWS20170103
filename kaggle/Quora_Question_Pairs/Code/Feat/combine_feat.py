...
def combine_feat(feat_names, feat_path_name):
    ...
    for run in range(1,config.n_runs+1):
        ...
        for fold in range(1,config.n_folds+1):
            ...
            for i,(feat_name,transformer) in enumerate(feat_names):
                ...
                # 填空
                x_train[np.isnan(x_train) + np.isinf(x_train)] = 0
                x_valid[np.isnan(x_valid) + np.isinf(x_valid)] = 0


                ## align feat dim
                dim_diff = abs(x_train.shape[1] - x_valid.shape[1])
                if x_valid.shape[1] < x_train.shape[1]:
                    x_valid = hstack([x_valid, np.zeros((x_valid.shape[0], dim_diff))]).tocsr() # hstack: 行对齐，列增加
    ...
    # Y_train = info_train["is_duplicate"]
    # # test
    # info_test = pd.read_csv("%s/test.info" % (save_path), encoding='utf-8')
