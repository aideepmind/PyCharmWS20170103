...
def set_scale_same(train_data):
    ...
    return gen_kfold_by_stratifiedKFold(train_data.toarray())
...
def hyperopt_obj(param, feat_folder, feat_name, trial_counter):
    ...
    if loaded is None:
        split_data = load_data(1, 1)
    log_loss_cv = np.zeros((config.n_runs, split_data.shape[0]), dtype=float)
    year = datetime.datetime.now().year
    for run in range(1, config.n_runs + 1):  # range(start, end)前包括后不包括
        for fold in range(1, split_data.shape[0] + 1):
            ...
            X_train, labels_train, X_valid, labels_valid = split_data[fold -1]
